...
func _generate_signal_flow_index():
	...
	for script_path in all_scripts:
		...
		for i in range(lines.size()):
			...
			on_event_regex.compile("@on_event\\(\"([^"]+)\"")
			...
			forward_signal_regex.compile("@forward_signal\\(\"[^"]+\"\,\s*\"([^"]+)\"")
			...
			emit_regex.compile("EventHub\.emit\(\"([^\"]+)\"")
			...
			subscribe_regex.compile("EventHub\.subscribe\(\"([^\"]+)\"")
...
func _scan_for_event_resources() -> Array[String]:
	...
	if dir:
		...
		while file_name != "":
			if dir.current_is_dir():
				if file_name != "addons" and file_name != ".git" and file_name != ".godot": # Avoid scanning unnecessary folders
